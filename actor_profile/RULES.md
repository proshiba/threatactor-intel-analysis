...
攻撃活動ではないActivity（法執行・妨害イベント）の扱いは4.3に従う。
...
逮捕、起訴、制裁指定、テイクダウン、インフラ差押えは、アクターの帰趨を左右する
重要事実であり、OSINT_RULES.mdの情報源優先順位でも最上位に位置する。記録しない
運用にはしない。ただしこれらは「アクターが攻撃した記録」ではないため、
攻撃活動と同じ扱いにもしない。

Activityとして保存し、`activity_type`は次のいずれかとする。

| 値 | 対象 |
|---|---|
| `law-enforcement-action` | 逮捕、起訴、訴追、有罪判決、制裁指定、報奨金設定 |
| `disruption-operation` | テイクダウン、ドメイン・サーバ差押え、シンクホール化 |

保存規則は次の通り。

1. `first_observed`と`last_observed`には、その法執行・妨害措置自体が実施された日を
   `source-stated`で入れる。資料が犯行期間だけを述べ措置日を明示しない場合は
   両方をunknownとし、犯行期間は`description`と`analyst_notes`へ記述する。
   犯行期間をこの活動の期間へ流用しない。措置日を攻撃観測日として扱わない。
2. これらの活動は攻撃活動の期間集計・並び替えから除外する。UIの「過去1年」
   「過去3年」、STIX Campaignの`first_seen`/`last_seen`、および
   `daily_check.py`の「直近に活動があったアクター」に含めない。除外しないと、
   摘発されたアクターほど直近の活動が活発に見える逆転が起きる。
3. IOCとTTPを起こさない。法執行資料は指標をほとんど公開せず、起訴状の罪状数や
   被害組織数から`victim_cases`を合成しない。被害範囲は資料が明示した範囲だけを
   `description`へ記述する。
4. 一次資料は法執行機関・司法当局の公式発表を優先し、報道はそれを補う位置づけと
   する。報道しか得られない場合は`confidence`を上げず、公式発表の未確認を
   `analyst_notes`へ残す。
5. `actor.active`は逮捕・起訴だけを根拠に`no`へ変更しない。資料が組織の解散、
   活動停止、または全構成員の拘束を明示した場合に限る。逮捕後も捜査継続、
   指導者交代、後継者による運用継続の記載がある場合は`active`を維持し、
   その旨を`actor.analyst_notes`と`assessment`へ残す。
6. 被疑者・被告人の氏名は、公的機関または一次報道が公表した範囲でのみ記述し、
   有罪認定と起訴内容を混同しない。
