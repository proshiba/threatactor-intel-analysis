...
- **詳細ページ** (`#/actor/<slug>[/タブ]`): `profiles/<slug>/actor-profile.json` を実行時に取得し、
  ...
  - **活動・被害**: 活動履歴のタイムラインと、活動・標的・マルウェア・TTP・
    ...
    活動を選択すると右側に活動別Diamond Model、攻撃元→標的地域、被害事例、根拠を表示し、
    タグは先頭10件だけを表示して残りを折りたたむ
...
- 活動別Diamond ModelはActivity、TTP、被害事例の明示参照から生成します。空の頂点を
  アクター全体の利用・標的実績で補完しないため、「情報なし」は活動単位の証拠不足を示します。
